...
test("Browser Fixture Test Two", async ({ page }) => {
    ...
    console.log(await page.locator("[style*='block']").textContent()); //textContent() - Used to extract test from the message
...
test.only("Login Practice - 1",async ({page}) => {
    ...
    // await page.waitForLoadState("networkidle"); // To overome the allTextContents() issue, we can use waitForLoadState("networkidle") which waits until the network calls halt
